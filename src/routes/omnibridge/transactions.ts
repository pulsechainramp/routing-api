...
type AuthenticatedTransactionsRequest = FastifyRequest<GetUserTransactionsRequest> & { user?: { sub?: string } };
...
const transactionsRateLimitMax = Number(process.env.OMNIBRIDGE_HISTORY_RATE_LIMIT_MAX ?? 30);
const transactionsRateLimitWindow = process.env.OMNIBRIDGE_HISTORY_RATE_LIMIT_WINDOW ?? '1 minute';
const transactionsRateLimitBan = Number(process.env.OMNIBRIDGE_HISTORY_RATE_LIMIT_BAN ?? 0);
...
export async function transactionRoutes(fastify: FastifyInstance, options: TransactionPluginOptions) {
  ...
  fastify.get<{ Querystring: GetUserTransactionsRequest['Querystring'] }>('/transactions', {
    preHandler: [
      fastify.authenticate,
      async (request: AuthenticatedTransactionsRequest, reply: FastifyReply) => {
    ...
    config: {
      rateLimit: (() => {
        const rateLimitConfig: any = {
          max: transactionsRateLimitMax,
          timeWindow: transactionsRateLimitWindow,
          keyGenerator: (req: any) => (req.user?.sub?.toLowerCase()) || getClientIp(req)
        };

        if (transactionsRateLimitBan > 0) {
          rateLimitConfig.ban = transactionsRateLimitBan;
        }
